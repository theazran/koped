...
const { liriklagu, quotemaker, randomNimek, fb, sleep, jadwalTv, ss, surat } = require('./lib/functions')
const { help, snk, info, donate, readme, listChannel, surah } = require('./lib/help')
const panggil = require('./lib/help')
...
let premium = JSON.parse(fs.readFileSync('./settings/banned.json'));
...
module.exports = msgHandler = async (client, message) => {
    try {
        const { author, type, id, from, t, sender, isGroupMsg, chat, caption, isMedia, mimetype, quotedMsg, quotedMsgObj, mentionedJidList } = message
        let { body } = message
                    ...
                    for (let i of limit){
                        if(i.id === id){
                            ...
                            if (limits >= limitCount) {
                                found = true;
                                client.reply(from, `Perintah ini hanya dapat dilakukan 10x :)
dan unlimited untuk member *Premium*`)
                                return true;
                            }else{
                                limit
                                found = true;
        ...
        const ownerNumber = '6285255646434@c.us'
        ...
        switch(command) {
        case '!bot':
        client.sendContact(from, '6285155343939@c.us')
        break
         case '!reg':
                          
                        if(!isOwner) return client.reply(from, 'Fitur Owner BOT')
                        premium.push(args[1]+'@c.us')
                        fs.writeFileSync('./settings/banned.json', JSON.stringify(premium))
                        client.sendText(from, 'Succes regis target!')

                            break
        case '!unreg':
        if(!isOwner) return
        let inx = premium.indexOf(args[1]+'@c.us')
                        premium.splice(inx,1)
                        fs.writeFileSync('./settings/banned.json', JSON.stringify(premium))
                        client.sendText(from, 'Succes unregg target!')
        break
        case '!limit':
        if (isPrem) {return client.reply(from, 'Sisa limit media Anda tersisa: *102727*')}
                        var found = false
                        for(let lmt of limit){
                            if(lmt.id === sender.id){
                                let limitCounts = limitCount-lmt.limit
                                if(limitCounts <= 0) return client.reply(from,`Limit media anda sudah habis`)
                                client.reply(from,`Sisa limit media anda tersisa : *${limitCounts}*`)
                                found = true
                            }
                        }
                        if (found === false){
                            let obj = {id: `${sender.id}`, limit:1};
                            limit.push(obj);
                            fs.writeFileSync('./settings/limit.json',JSON.stringify(limit, 2));
                            client.reply(`Sisa limit media anda tersisa : *${limitCount}*`)
                        }
                        break
        case '!sticker':
        case '!stiker':
        if (isLimit(sender.id)) return
    ...
    case '!nobg':
		...
		//untuk api key kalian bisa dapatkan pada website remove.bg
                var result = await removeBackgroundFromImageBase64({ base64img, apiKey: 'giKZ4Kn4LYnU5UyvaUWXx64m', size: 'auto', type: 'auto', outFile })
        ...
        case '!donasi':
        case '!donate':
            client.reply(from, 'Mau donasi kak? Private chat aja ya biar enak ngomongnya :)')
            break
        case '!tts':
        ...
        if (isLimit(sender.id)) return
            ...
            const teks2 = body.slice(7)
            client.reply(from, mess.wait, id)
            const splitTeks = teks2.replace(/(\S+\s*){1,10}/g, '$&\n')
            const fixHeight2 = splitTeks.split('\n').slice(0, 25).join('\n')
            spawn('convert', [
                './media/img/aa.jpeg',
                '-font',
                ...
                '+120+140',
                fixHeight2,
                './media/img/after2.jpeg'
            ])
            .on('error', () => client.reply(from, 'Error gan', id))
            .on('exit', () => {
                limitAdd(sender.id)
                client.sendImage(from, './media/img/after2.jpeg', 'nulis2.jpg', 'Nih mhank', id)
            })
            break
        ...
        case '!me':
            ...
            if (isGroupMsg) {
              if (!quotedMsg) {
              var block = premium.includes(author)
             ...
             await client.sendFileFromUrl(from, pfp, 'pfp.jpg', `*USER PROFILE*\n\nNama: ${namae}\nStatus: ${status}\nPremium: ${block}\nAdmin: ${adm}`)
             } else if (quotedMsg) {
             var qmid = quotedMsgObj.sender.id
             var block = premium.includes(qmid)
            ...
            // await client.sendFileFromUrl(from, pfp, 'pfo.jpg', `*User Profile*\n\n 🔖️ *Username: ${namae}*\n💌️ *User Info: ${status}*\n*💔️ Ban: ${block}*\n✨️ *Role: ${role}*\n 👑️ *Admin: ${adm}*`)
        ...
        case '!infogroup' :
            ...
            if (!isGroupMsg) return client.reply(from, 'Ini bukan grup bambang...') 
            var inviteLink = await client.getGroupInviteLink(groupId);
            var totalMem = chat.groupMetadata.participants.length
            var desc = chat.groupMetadata.desc
            var groupname = chat.contact.name
...
break
    ...
    if (!isGroupMsg) return
        ...
        await client.reply(from, `*${chat.contact.name}*
...
*instagram.com/theazran_*`)
    break
        ...
        /*case '!fb':
            ...
            break*/
        case '!creator':
            client.sendLinkWithAutoPreview(from, `https://instagram.com/theazran_\n\n Difollow ya kak!`)
            break
        /*case '!ig':
        ...
        case '!demote':
            ...
            break
        case '!join':
            if (!isPrem) return client.reply(from, 'Maaf fitur ini hanya untuk member Premium, silahkan private chat jika ingin daftar!')
            ...
            if (tGr.length > 1) return client.reply(from, 'Maaf jumlah group sudah maksimal!', id)
        ...
        case '#menu':
        case '#help':
            client.sendText(from, panggil.textMenu(pushname))
            break
        case '!petunjuk':
            client.reply(from, readme, id)
            break
        case '!info':
            client.reply(from, 'BOT ini dibuat dengan bahasa pemrograman Node.js/Javascript.')
            break
        // case '!snk':
